...
<table style="padding: 5px;margin:0; border-spacing: 0px" width="100%">
	...
	<tr>
		...
		<td colspan="2" align="left" style="#000; padding: 0px; border:0px; padding: 20px;" width="60%">
			...
			<h5>SUCURSAL: <strong><small style="font-size: 16px"># TIENDA</small></strong></h5>
		</td>
	</tr>
</table>
...
<table style="padding: 5px;margin:0; border-spacing: 0px;  font-size: 12px" width="100%">
	...
	<tbody >
		...
		<?php $tem_row = 0; ?>
		<?php foreach (ViewVenta::getListaCompraProductoPreCaptura(4) as $key => $producto): ?>
			<?php if ($tem_cliente_count < 0 || $tem_cliente_count == $tem_row): ?>
				<?php $tem_cliente_count = ViewVenta::getCountPreventaCliente(4,$producto["cliente_id"]) ?>
				<?php $tem_row = 0; ?>
			<?php endif ?>
			<?php $tem_row++; ?>
...
<table style="padding: 5px;margin:0; border-spacing: 0px;  font-size: 12px" width="100%">
	<thead>
		<tr >
			...
			<th align="center" width="15%" style="border-style:  solid;border-width: 1px;border-spacing: 0px; ">CANTIDAD TOTAL SOLICITADA</th>
			<th align="center" width="15%" style="border-style:  solid;border-width: 1px;border-spacing: 0px; ">CANTIDAD TOTAL INVENTARIO</th>
	...
	<tbody >
		...
		<?php foreach (ViewVenta::getListaCompraProductoPreCapturaGroup(4) as $key => $producto): ?>
			<?php $count++ ?>
