...
<table style=" border-spacing: 0px;" width="100%">
	<tr>
		<td width="50%">
			<p style="font-size:14px"><strong>CLIENTE:</strong> <?= $cliente->nombreCompleto  ?></p>
			<h5><strong>Vendedor / Empleado:</strong><?= $reparto->createdBy->nombreCompleto ?> </h5>
		</td>
		<td width="50%">
			<p style="font-size:14px"> <strong>NOTAS DE VENTA : </strong>
				[<?php foreach ($foliosArray as $key => $item_folio): ?>
					#<?= str_pad($item_folio,6,"0",STR_PAD_LEFT) ?>,
				<?php endforeach ?>]</p>
			<p style="font-size:14px"> <strong>FECHA DE REPARTO : </strong> <?= date("Y-m-d",$reparto->created_at) ?></p>
			<h5><strong>Ruta:</strong> <?=  $reparto->sucursal->nombre ?></h5>
		</td>
	</tr>
</table>
...
<table style="padding: 5px;margin:0; border-spacing: 0px;  font-size: 12px" width="100%">
	...
	<tbody >
		...
		<?php foreach ($detail as $key => $ventaItem): ?>
			<tr>
				<td align="center" ><?= $ventaItem["cantidad"] ?></td>
				<td align="center" ><?= Producto::$medidaList[$ventaItem["tipo_medida"]]  ?> </td>
				<td align="center" ><?= $ventaItem["clave"] ?></td>
				<td align="center" ><?= $ventaItem["nombre"] ?></td>
...
<table style="padding-bottom: 5px;margin:0; border-spacing: 0px;" width="100%">
	<tr>
		<td width="50%" align="right"><h5 style="font-weight:bold">TOTAL</h5></td>
		<td width="50%" align="center" style="border-bottom-style:solid; border-width: 2px;">
			 <p style="font-size:16px; font-weight: bold;">$<?= number_format($total,2) ?></p>
		</td>
	</tr>
	<tr>
		<td></td>
