...
<table style="padding: 5px;margin:0; border-spacing: 0px" width="100%">
	...
	<tr>
		...
		<td colspan="2" align="left" style="#000; padding: 0px; border:0px; padding: 20px;" width="60%">
			...
			<h5>SUCURSAL: <strong><small style="font-size: 16px"># <?= $sucursal->nombre ?></small></strong></h5>
		</td>
	</tr>
</table>
...
<table style="padding: 5px;margin:0; border-spacing: 0px;  font-size: 12px" width="100%">
	...
	<tbody >
		...
		<?php $tem_row = 0; ?>
		<?php foreach (ViewVenta::getListaCompraProductoPreCaptura($sucursal->id, $ruta_id) as $key => $producto): ?>
			<?php if ($tem_cliente_count < 0 || $tem_cliente_count == $tem_row): ?>
				<?php $tem_cliente_count = ViewVenta::getCountPreventaCliente($sucursal->id,$producto["cliente_id"], $ruta_id) ?>
				<?php $tem_row = 0; ?>
			<?php endif ?>
			<?php $tem_row++; ?>
			<tr style="border-style:  solid;border-width: 1px;border-spacing: 0px;">
				...
				<td align="center" style="border-style:  solid;border-width: 1px;border-spacing: 0px;"><?= $producto["total_producto"].' '.Producto::$medidaList[$producto["tipo_medida"]]  ?></td>
...
<table style="padding: 5px;margin:0; border-spacing: 0px;  font-size: 12px" width="100%">
	...
	<tbody >
		...
		<?php foreach (ViewVenta::getListaCompraProductoPreCapturaGroup($sucursal->id, $ruta_id) as $key => $producto): ?>
			<?php $count++ ?>
