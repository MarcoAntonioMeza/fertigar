...
<div class="creditos-credito-view">
    <div class="row">
        <div class="col-md-6" style="height: 550px;">
            <div class="ibox" >
                ...
                <div class="ibox-content text-center">
                    ...
                    <p class="font-bold">Tel : <?= $model->telefono_movil  ?> / <?= $model->tel ?></p>
                </div>
            </div>
            <div class="panel">
                <div class="panel-body text-center">
                    <?php $totales = Credito::getTotalesCredito( $model->id , Credito::TIPO_PROVEEDOR ) ?>
                ...
                <div class="ibox-content" style="height: 450px;overflow-y: auto;">
                    <div class="pagos-vigente-index" >
                        <div class="btt-toolbar" >
                            <?= Html::hiddenInput('tipo', Credito::TIPO_PROVEEDOR ) ?>
        ...
        <div class="col-md-6"  style="height: 550px;">
            <div class="ibox">
                <div class="ibox-title">
                    <h5 >OPERACIONES DE PAGO</h5>
                </div>

                <div class="ibox-content text-center" style="height:470px; overflow-y: auto;">
                    <table class="table table-hover issue-tracker">
                        <tbody>
                            <?php foreach (Credito::getHistoryAbonos($model->id) as $key => $item_pago): ?>
                            <tr>
                                <td>
                                    <span class="label label-<?= $item_pago["status"] == CreditoAbono::STATUS_ACTIVE ? 'primary' : 'danger'  ?> ">ABONO [ <?= $item_pago["status"] == CreditoAbono::STATUS_ACTIVE ? 'VIGENTE' : 'CANCELADO'  ?> ]</span>
                                </td>
                                <td class="issue-info">
                                    <a href="javascript::void()" style="color: #007bff; text-decoration: none;">
                                        CREDITO # <?= $item_pago["credito_id"] ?>
                                    </a>
                                    <small>
                                        Abono al credito <strong>#<?= $item_pago["credito_id"] ?></strong> con el empleado <strong><?= $item_pago["created_by_user"] ?></strong>
                                    </small>
                                </td>
                                <td>
                                    <h3><strong>$<?= number_format($item_pago["cantidad"],2) ?></strong></h3>
                                </td>
                                <td>
                                    <?= date("Y-m-d h:i", $item_pago["created_at"]) ?>
                                </td>
                            </tr>
                            <?php endforeach ?>
                        </tbody>
             ...
             <div class="ibox">
                ...
                <div class="ibox-content">
                    <table class="table">
                        ...
                        <tbody>
                            <?php foreach (Credito::getCredito(  $model->id , Credito::TIPO_PROVEEDOR ) as $key => $item_credito): ?>
                                <tr class="text-center" style="<?= $item_credito->status == Credito::STATUS_PAGADA ? 'background-color: #457c6a;color: #fff;' : ( $item_credito->status == Credito::STATUS_CANCEL ? 'background-color: #b75252;color: #fff;' : '')?>">
                                    ...
                                    <td><?= Html::a("#".$item_credito->compra_id, ['/compras/compra/view' , "id" => $item_credito->compra_id ], [ "style" => "font-size: 16px;color: #eed522;font-weight: bold;" ]) ?></td>
                                    ...
                                    <td><?= date("Y-m-d h:i:s",$item_credito->created_at)?></td>
                                    <td>
