...
$this->title = 'PAGO DE CREDITO [PROVEEDOR]';
$this->params['breadcrumbs'][] = $this->title;
$bttExport    = Yii::$app->name . ' - ' . $this->title . ' - ' . date('Y-m-d H.i');
$bttUrl       = Url::to(['get-history-operacion-proveedor-global']);
$bttUrlView   = Url::to(['view?id=']);
?>

<div class="creditos-register-pay">
    <div class="row">
        <div class="col-sm-12">
            ...
            <div class="ibox">
                <div class="ibox-title">
                    <h3>SELECCIONA AL PROVEEDOR</h3>
                </div>
                <div class="ibox-content">
                    <?= Select2::widget([
                        'id' => 'proveedor-proveedor_id',
                        'name' => 'proveedor-proveedor_id',
                        'data' => [],
                        'pluginOptions' => [
                            'allowClear' => true,
            ...
            <div class="tabs-container">
                <ul class="nav nav-tabs" role="tablist">
                    ...
                    <li>
                        <a class="nav-link" data-toggle="tab" href="#tab-historial-pago-global">HISTORIAL DE ABONOS [GLOBAL]</a>
                    </li>
                </ul>
                <div class="tab-content">
                            ...
                            <div class="panel-body">
                                ...
                                <div class="table_credito" >
                                    ...
                                    <div class="div_metodo_otro" style="padding:15px;display: none;">
                                        ...
                                        <?php /* ?>
                                        <?php if (Sucursal::getMySucursal()): ?>
                                            <h2>SUCURSA <strong>[<?= Sucursal::getMySucursal()->nombre ?>]</strong></h2>
                                        <?php else: ?>
                                            <h2 class="text-danger">NO TIENES SUCURSA ASIGNADA, NO PODRAS REALIZAR ESTE TIPO DE OPERACIÓN</h2>
                                        <?php endif ?>
                                        <p><strong class="text-danger">* Especifica el producto y cantidad </strong></p>
                                        <div class="row ">
                                            <div class="col-sm-6">
                                            ...
                                            <div class="col-sm-6">
                                                <strong>CANTIDAD<strong class="text-danger lbl_tipo_producto">[]</strong></strong>
                                                <?= Html::input("number",null,false,[ "class" => "form-control", "id" => "input-cantidad-recibe", 'autocomplete'=> 'off', 'style' => 'font-size: 24px;font-weight: bold;', "step" => "0.001"]) ?>
                                            </div>
                                        </div>*/?>
                                    ...
                                    <div class="form-group" style="padding:15px">
                                        ...
                                        <?= Html::submitButton('INGRESAR PAGO', ['class' => 'btn btn-primary btn-lg col-sm-3', "style" => "font-size:20px","id" => "btnCreditoAdd"]) ?>
                    ...
                    <div id="tab-historial-pago-global"  role="tabpanel" class="tab-pane xxxx">
<!--                        <div class="panel">-->
<!--                            <div class="panel-body">-->
<!--                                <table class="table table-bordered">-->
<!--                                    <thead>-->
...
<script>
    $(document).ready(function(){
        ...
        var columns = [
                ...
                },
                {
                    ...
                    switchable: false,
                    sortable: true,
                    formatter: btf.historial_bonos_proveedores.token_pay,
                },
                {
                    field: 'cantidad',
                    title: 'ABONO',
    ...
    var $inputSearchCliente = $("#proveedor-proveedor_id"),
    ...
    $inputSearchCliente.change(function(){
        ...
        if ($(this).val()) {
            ...
            $('.table_credito').show();
            $.get("<?= Url::to(['get-credito-proveedor']) ?>",{ proveedor_id : $(this).val() },function($response){
    ...
    var render_notas_credito = function(){
        ...
        $.each(notaCreditoArray,function(key, credito){
            content_html += "<tr>"+
                "<td class='text-center'>"+ (credito.id ? "<a href='"+ URL_PATH +"/creditos/credito/view?id=" + credito.id +"' target='_black' style='font-size:24px; font-weight: bold' >"+ credito.id +"</a>" : "---" )+"</td>"+
                "<td class='text-center'>"+ (credito.compra_id ? "<a href='"+ URL_PATH +"/compras/compra/view?id=" + credito.compra_id +"' target='_blank' style='font-size:24px; font-weight: bold'>#"+ credito.compra_id +"</a>" : "---")+"</td>"+
    ...
    var onGetOperacion = function( $proveedor_id ){
        $contentHtml.html(null);
        $contentHtmlGlobal.html(null);
        $.get('<?= Url::to(["get-history-operacion-proveedor"]) ?>',{ proveedor_id : $proveedor_id },function($response){
            if ($response.code == 202 ) {
                $tempHtml = "";
        ...
        //$.get('<?//= Url::to(["get-history-operacion-proveedor-global"]) ?>//',{ proveedor_id : $proveedor_id },function($response){
        //    if ($response.code == 202 ) {
        //        $tempHtmlGlobal = "";
        //        $.each($response.transaccion,function(key, item_transaccion){
        //            $tempHtmlGlobal += "<tr>"+
    ...
    var open_ticket = function(token_pay){
        window.open("/web/creditos/credito/imprimir-credito" + "?pay_items=" + token_pay
        ...
        $.each(metodoPago_arrayCredito, function(key, metodo){
            if (metodo.metodo_id) {
                ...
                if (metodo.metodo_pago_id == VAR_DEBITO_CREDITO || metodo.metodo_pago_id == VAR_TARJETA_CREDITO)
                    ...
                    $("#table_credito_metodo_cantidad",$tr).html( btf.conta.money( metodo.cantidad) +" + [ PRODUCTO - "+ metodo.producto_text + " / cantidad recibida: "+ metodo.cantidad_recibe +"]");
                else
    ...
    $btnAgregarMetodoPagoCredito.click(function(){
        ...
        //if ( parseFloat($('#inputCantidadCredito').val()) >= pago_total   ) {
            ...
            metodo = {
                ...
                //"cantidad"          : $form_metodoPagoCredito.$cantidad.val(),
                //"cargo_extra"       : $form_metodoPagoCredito.$metodoPago.val() == VAR_TARJETA_CREDITO || $form_metodoPagoCredito.$metodoPago.val() == VAR_DEBITO_CREDITO  ? (3 * parseFloat($form_metodoPagoCredito.$cantidad.val())) / 100 : 0,
                ...
                //"cantidad_recibe"   :  $form_metodoPagoCredito.$metodoPago.val() == VAR_COBRO_OTRO  ? $inputCantidadRecibe.val() : null,
        ...
        /*}else{
            ...
            $('.text-message-credito').html('El monto ingresado debe ser manor, intente nuevamente');
        }*/
    ...
    $form_metodoPagoCredito.$metodoPago.change(function(){
     ...
     $btnCreditoAdd.on('click',function(event){
        ...
        if (creditoArray.length > 0) {
            ...
            if ( parseFloat($('#inputCantidadCredito').val()) ==  0 ) {
                $('.alert-credito-error').html('La monto a pagar debe ser MAYOR A 0 !');
            ...
            if ( parseFloat($('#inputCantidadCredito').val()) < total ) {
                $('.alert-credito-error').html("El monto a pagar debe ser de : $" + total + ", ingresa el monto correctamente");
                $('.alert-credito-error').show();
                $btnCreditoAdd.attr('disabled',false);
                hide_loader();
                return false;
            ...
            if (total > total_ingresado ) {
                $('.alert-credito-error').html("El residuo debe ser $0.00, ingresa los pagos correspondientes");
                $('.alert-credito-error').show();
                $btnCreditoAdd.attr('disabled',false);
                hide_loader();
                return false;
            ...
            $.post("<?= Url::to(['post-credito-create']) ?>",{ listCredito : creditoArray, total : total, metodoPagoArray : metodoPago_arrayCredito  },function($response){
                if ($response.code == 202) {
                    ...
                    $('.alert-credito-error').html("Ocurrio un error al procesar el pago, intenta nuevamente");
                    $('.alert-credito-error').show();
                    $btnCreditoAdd.attr('disabled',false);
                    hide_loader();
                }
            ...
            $('.alert-credito-error').html("Debes seleccionar un credito para generar un PAGO.");
            $('.alert-credito-error').show();
            $btnCreditoAdd.attr('disabled',false);
            hide_loader();
        }
